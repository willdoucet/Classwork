...
def welcome():
    ...
    return (
        ...
        f"api/v1.0/tobs<br/>"
        f"api/v1.0/start   -- enter start date using format yyyy-mm-dd<br/>"
        f"api/v1.0/start/end -- enter start and end date using format yyyy-mm-dd<br/>"
        )
...
def stations():
    ...
    station_obs = session.query(Station.station, Station.name, Station.latitude, Station.longitude, Station.elevation).all()
    station_list = []
...
def tempcalc(start):
    ...
    date_str= f"displaying temp data for period {str(start_date)} to 2017-08-23"
